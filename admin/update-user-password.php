...
if(isset($_POST['submit']))
{
    ...
    if (empty($_POST['current_password']) || empty($_POST['new_password']) || empty($_POST['confirm_password'])) {
        $_SESSION['update-password-user'] = "<h3 class='error'>Fill Up All The Field !!!</h3>";
        header('location:update-user-password.php?id=' . $id);
        die();
    } else if ($pwd != $currentPassword) {
        $_SESSION['update-password-user'] = "<h3 class='error'>Old Password Not Match !!!</h3>";
        header('location:update-user-password.php?id=' . $id);
        die();
    } else {
        if (!empty($_POST['new_password']) && strlen($_POST['new_password']) < 8) {
            $_SESSION['update-password-user'] = "<h3 class='error'>Password must be at least 8 characters long !!!</h3>";
            header('location:update-user-password.php?id=' . $id);
            die();
        }
        if ($newPassword != $confirmPassword) {
            $_SESSION['update-password-user'] = "<h3 class='error'>New Password and Confirm Not Match !!!</h3>";
            header('location:update-user-password.php?id=' . $id);
            die();
        } else {
            ...
            if ($result2 == TRUE) {
                $_SESSION['update-password-user'] = "<h2 class='success'>User Password Updated Successfully</h2>";
